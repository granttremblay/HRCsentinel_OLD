...
#    J. Chappell (jhc)
#
#    Program: CLP-032-chk_proper_det_vs_rampupdown_scs.pl
#    Purpose: 
#    External files:
#    External programs:
#
...
{ ### start of main program
	...
	#		if( $b[1] == 93 ){ $SV{HVS} = 'on'; }
		...
		if( ($L =~ m/2PRAISL/) ){
			$PREAMPA{State} = "I";
			$HV{$det}{Line}  = $L;
			#print "PREAMPA $PREAMPA{State}  at:$L\n";

			$scs = 90;
			if( ($PREAMPA{State} =~ m/I/) && ($SCS{$scs}{State} =~ m/ena/) ){
				$ne++;
				$ERR{$ne} = "$a[0]: PREAMPA = I and  SCS($scs) HRC-S HV Ramp Up is enabled ";
			}
		}
		...
		if( ($L =~ m/2PRASSL/) ){
			...
			$scs = 89;
			if( ($PREAMPA{State} =~ m/S/) && ($SCS{$scs}{State} =~ m/ena/) ){
				$ne++;
				$ERR{$ne} = "$a[0]: PREAMPA = S and  SCS($scs) HRC-I HV Ramp Up is enabled ";
			}
		}
	...
	print "CLP-032-chk_proper_det_vs_rampupdown_scs.pl: ";
	if( $ne == 0 ){
		print "[OK]\n";
	}else{
		print "[FAIL]\n";
...
sub GetIP {
	...
	$$rv{Program}      = "CLP-032-chk_proper_det_vs_rampupdown_scs.pl";
	$$rv{Subroutine}   = "GetIP";
	$$rv{Section}      = "";
	$$rv{ErrorMessage} = "";
	$$rv{ReturnValue}  = 0;
	...
	$$ip{Prog}        = 'CLP-032-chk_proper_det_vs_rampupdown_scs.pl';
	$$ip{Description} = 'Check for proper scs ramp up/down selection .';
	$$ip{CmdLine}     = "@ARGV";
	$$ip{Version}     = '$Rev$';
	$$ip{PrintVersion}= 0;
	$$ip{DebugLevel}  = 0;
