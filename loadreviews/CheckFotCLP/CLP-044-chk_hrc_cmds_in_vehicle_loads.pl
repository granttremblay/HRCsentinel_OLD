...
#    J. Chappell (jhc)
#
#    Program: CLP-044-chk_hrc_cmds_in_vehicle_loads.pl
#    Purpose: Check for hrc valid commands in vehicle load products
#    External files:
#    External programs:
#
#    Copyright:   
...
{ ### start of main program
	...
	# Create the vehicle valid commad list
	@VALIDCMDLIST = qw( 2NXILASL 2FIFOAON 2FIFOAOF 2OBSVASL);
	...
	foreach $L (@HRCSEL){
		...
		if( $L =~ m/ == / ){ 
			$p = 0;
			for $validcmd (@VALIDCMDLIST){
				if( $validcmd =~ m/\Q$a[3]/ ){ $p++; }
			}
			if( $p == 0 ){
				$ne++;
...
sub GetIP {
	...
	$$rv{Program}      = "CLP-044-chk_hrc_cmds_in_vehicle_loads.pl";
	$$rv{Subroutine}   = "GetIP";
	$$rv{Section}      = "";
	$$rv{ErrorMessage} = "";
	$$rv{ReturnValue}  = 0;
	...
	$$ip{Prog}        = 'CLP-044-chk_hrc_cmds_in_vehicle_loads.pl';
