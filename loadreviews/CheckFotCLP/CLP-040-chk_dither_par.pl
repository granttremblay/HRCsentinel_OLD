...
#    J. Chappell (jhc)
#
#    Program: CLP-040-chk_dither_par.pl
#    Purpose: 
#    External files:
#    External programs:
#
...
{ ### start of main program
	...
	foreach $L (@HRCSEL){
		...
		### first time thru, check for DITHER PARAMETERS
		if( ($L =~ m/DITHER_PAR = /) && ($ftt == 1) ){
			@b = split('=', $L);
			@c = split(':',$b[1]);
			$ANGP   =  $c[0];
			$ANGP =~ s/ +//g;
	...
	#		if( $b[1] == 93 ){ $SV{HVS} = 'on'; }
		...
		if( $L =~ m/DITHER ENABLE/ ){
				...
				@b = split(" ",$L);
				print "Dither is Enabled @ $b[0] vcdu=$b[2]\n";
				...
				if( ($SCS{91}{State} =~ m/act/) && ($OBS{SI} =~ m/HRC/) ) {
					print "  scs91 is active AND dither is enabled - Checking Dither parameters:\n";
...
sub GetIP {
	...
	$$rv{Program}      = "CLP-040-chk_dither_par.pl";
	$$rv{Subroutine}   = "GetIP";
	$$rv{Section}      = "";
	$$rv{ErrorMessage} = "";
	$$rv{ReturnValue}  = 0;
	...
	$$ip{Prog}        = 'CLP-040-chk_dither_par.pl';
	$$ip{Description} = 'Check for proper dither parameters for observation.';
	$$ip{CmdLine}     = "@ARGV";
	$$ip{Version}     = '$Rev$';
	$$ip{PrintVersion}= 0;
	$$ip{DebugLevel}  = 0;
